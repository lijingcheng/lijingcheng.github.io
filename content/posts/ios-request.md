...
# app 在基于第三方网络库后的网络层设计

## 接口安全
## 分页接口的设计
1、只在必要的情况下发起请求，当数据不符合规范等客户端能够处理的情况时客户端应该给用户提示，而不是发起请求，由后台提示用户
2、要对接口返回的json数据有容错处理，不能因数据不规范导致崩溃
3、如果用swift的model,enum，还需要考虑数据容错，当返回客户端未定义的数据时要能够设置指定默认值（贴下代码）
### 对于分页接口，app要支持两种接口（介绍下 UIScrollViewExt 中自动计算 pageNo 方案。。。。）
#### 按页号分，app 需要向接口提供 page 和 limit 字段，告诉 app 是要第几页数据并且要多少条，app 需要接口返回 total 字段，好根据数据总数以前当前的 page 和 limit 字段计算是否还需要请求下一页数据。但按页号分页不适合数据更新快和用户可以主动删除数据的业务场景，否则会导致数据错乱。

#### 根据标识符分页，app 需要向接口提供 offset 和 limit 字段，告诉 app 上次拿到的数据中最后一条数据的 id 或时间戳是什么，用来标识从后端应该从哪里开始获取新数据，并且要多少条
